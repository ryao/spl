...
#define	KM_ZERO		0x8000	/* zero the allocation */
...
{
	gfp_t	lflags = __GFP_NOWARN | __GFP_NORETRY;
...
/*
 * The vmem_* functions map to a low level interface on Solaris that has no
 * equivalent available to us on Linux. Effort is made to provide the benefit
 * of using vmem_* to the kmem_* equivalents, so we can harmlessly map these to
 * them.
 */
...
enum {
	...
	KMC_BIT_TOTAL		= 20,	/* Proc handler helper bit */
	KMC_BIT_ALLOC		= 21,	/* Proc handler helper bit */
	KMC_BIT_MAX		= 22,	/* Proc handler helper bit */
};
...
#define kmem_virt(ptr)			is_vmalloc_addr(ptr)
